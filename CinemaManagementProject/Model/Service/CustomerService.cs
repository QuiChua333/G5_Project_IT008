        ...
        public async Task<List<CustomerDTO>> GetAllCustomerByTime(int year, int month = 0)
        {
            try
            {
                ...
                if (month == 0)
                {
                    using (var context = new CinemaManagementProjectEntities())
                    {
                        var customer = await context.Customers.Where(c => !(bool)c.IsDeleted && c.FirstDate.Year == year).Select(c => new CustomerDTO
                        {
                            Id = c.Id,
                            ...
                            Expense = c.Bills.Where(b => b.CreateDate.Year == year).Sum(b => (double?)b.TotalPrize) ?? 0
                        }).ToListAsync();
                    ...
                    using (var context = new CinemaManagementProjectEntities())
                    {
                        var customer = await context.Customers.Where(c => !c.IsDeleted && c.FirstDate.Year == DateTime.Today.Year && c.FirstDate.Month == month)
                            .Select(c => new CustomerDTO
                            {
                                ...
                                Email = c.Email,
                                FirstDate = c.FirstDate,
                                Expense = c.Bills.Where(b => b.CreateDate.Year == year && b.CreateDate.Month == month).Sum(b => (double?)b.TotalPrize) ?? 0
                            }).ToListAsync();
                ...
                using (var context = new CinemaManagementProjectEntities())
                {
                    ...
                    if (cus != null)
                    {
                        ...
                        else
                        {
                            ...
                            cus.CustomerCode = newCus.CustomerCode;
                        ...
                        return (true, "Đăng ký thành công", cus.CustomerCode);
                    }


                    string currentMaxCode = await context.Customers.MaxAsync(c => c.CustomerCode);
                    Customer newCusomer = new Customer
                    {
                        CustomerCode = CreateNextCustomerCode(currentMaxCode),
                    ...
                    await context.SaveChangesAsync();
                    return (true, "Đăng ký thành công", newCusomer.CustomerCode);
                }
            }
            catch (Exception e)
            {
